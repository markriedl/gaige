...
def rayTraceWorld(p1, p2, worldLines):
	if thisWorld:
		return rayTraceWorldOther(p1, p2, thisWorld)
	for l in worldLines:
...
def rayTraceWorldOther(p1, p2, world):
	return rayTraceWorldCustom(p1, p2, world, rayTrace)

def rayTraceWorldCustom(p1, p2, world, rayFunc):
	x = 0
	y = 0
	w = 0
	h = 0
	...
	for o in world.quadTree.hit(rect):
		for l in o.getLines():
			hit = rayFunc(p1, p2, l)
			if hit != None:
				return hit

	for l in world.getOtherLines():
		hit = rayFunc(p1, p2, l)
		if hit != None:
			return hit
	return None
...
def rayTraceWorldNoEndPoints(p1, p2, worldLines):
	if thisWorld:
		return rayTraceWorldCustom(p1, p2, thisWorld, rayTraceNoEndpoints)
	for l in worldLines:
